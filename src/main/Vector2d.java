...
public class Vector2d extends Object {
   ...
   final public int y;
    ...
    public static Vector2d getRandomPointFromArea(Vector2d leftBottomCorner, Vector2d rightUpperCorner)
    {
    	...
    	int x = leftBottomCorner.x+(generator.nextInt()%(rightUpperCorner.x-leftBottomCorner.x));
    	int y = leftBottomCorner.y+(generator.nextInt()%(rightUpperCorner.y-leftBottomCorner.y));
    ...
    {
    	Random generator = new Random();
    	int howManyTimes = generator.nextInt()%8;
    	Location location = new Location(position, MapDirection.NORTH);
    	location.rotate45DegClockwise(howManyTimes);
